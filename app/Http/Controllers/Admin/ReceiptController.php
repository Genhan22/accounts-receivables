...
class ReceiptController extends Controller
{
    ...
    public function store(ReceiptFormRequest $request)
    {
        ...
        $receipts->studentType = $validatedData['studentType'];
        $receipts->scholarship = $validatedData['studentType'] == 'undergraduate' ? $request->scholarship : null;
    ...
    public function update(ReceiptFormRequest $request, int $ReceiptID)
    {
        ...
        if ($receipts) {
            $receipts->update([
                ...
                'DatePaid' => $validatedData['DatePaid'],
                'studentType' => $validatedData['studentType'],
                'scholarship' => $validatedData['studentType'] == 'undergraduate' ? $request->scholarship : null
            ]);

            return redirect('admin/receipts')->with('message', 'Receipt Updated Successfully');
        } else {
